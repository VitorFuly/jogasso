...
void LoadGame(GameState *game) {
	...
	for (int i = 0; i < 25; i++) {
		if (i == 5) {
			game->piso[i].h = 95;
			game->piso[i].w = 160;
			game->piso[i].x = 8000 * (float) 160.0;
			game->piso[i].y = 650;
		}
		if (i == 6) {
			game->piso[i].h = 95;
			game->piso[i].w = 160;
			game->piso[i].x =9000 * (float) 160.0;
			game->piso[i].y = 650;
		}
	...
	game->inimigo[16].xe = 9200;
	game->inimigo[16].ye = 580;
	game->inimigo[16].we = 40;
	game->inimigo[16].he = 60;
	game->inimigo[16].baseX = game->inimigo[16].xe;
	game->inimigo[16].baseY = game->inimigo[16].ye;
	game->inimigo[16].phase = 2;
	game->inimigo[16].mode = 0;
	game->inimigo[16].aocontrario = 0;
	game->inimigo[16].dxe = 10;
	game->inimigo[16].mov2 = 4;
...
void colisaoDetect(GameState *game) {
	...
	for (int i = 0; i < 20; i++)
	{
		...
		if (collide2d(game->man.x, game->man.y, game->voador1[i].x, game->voador1[i].y, 74, 94, 40, 40))
		{
			if (game->voador1[i].vivo == 1)
			{
				game->score -= 100;
...
void doRender(SDL_Renderer *renderer, GameState *game)
{
	...
	if (game->statusState == STATUS_STATE_MENU) {
		...
		for (int i = 0; i < 20; i++) {
			if (game->voador1[i].vivo == 1) {
				SDL_Rect voador1Rect = { game->scrollx + game->voador1[i].x*1.0f,game->voador1[i].y,40,40 };
				SDL_RenderCopyEx(renderer, game->voador[game->voador1[1].mov], NULL, &voador1Rect, 0, NULL, game->voador1[i].aocontrario == 0);
			}
		}
		...
		//piso
			...
			for (int k = 0; k < 65; k++) {
				...
				if (fabs(game->piso[k].x - game->man.x) < 1350) {
					SDL_Rect pisoRect = { game->scrollx + game->piso[k].x*1.0f, game->piso[k].y*1.0f,game->piso[k].w * 1.0f, game->piso[k].h*1.0f };
			...
			for (int u = 0; u < 70; u++) {
				if (fabs(game->plataform[u].x - game->man.x) < 1350) {
					SDL_Rect platRect = { game->scrollx + game->plataform[u].x*1.0f, game->plataform[u].y*1.0f,game->plataform[u].w + 10 * 1.0f, game->plataform[u].h + 15 * 1.0f };
			...
			for (int l = 30; l < 65; l++) {
				if (fabs(game->piso2[l].x - game->man.x) < 1350) {
					SDL_Rect piso1Rect = { game->scrollx + game->piso2[l].x*1.0f, game->piso2[l].y*1.0f,game->piso[l].w * 1.0f, game->piso[l].h*1.0f };
			...
			for (int p = 0; p < 70; p++) {
				...
				if (fabs(game->plataforma2[p].x - game->man.x) < 1350) {
					SDL_Rect plat2Rect = { game->scrollx + game->plataforma2[p].x*1.0f, game->plataforma2[p].y*1.0f,game->plataform[p].w + 10 * 1.0f, game->plataform[p].h + 15 * 1.0f };
