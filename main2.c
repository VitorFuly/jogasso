	...
	while (SDL_PollEvent(&event))
	{
		...
		case SDL_KEYDOWN:
		{
			...
			case SDLK_f:
				...
				printf("%d %d %d %d %d %d\n", game->sobvoltarfim, game->sobvoltargo, game->sobsairfim, game->sobsairgo, game->sobvoltarm, game->sobsairfim);
...
void doRender(SDL_Renderer *renderer, GameState *game)
{
	...
	if (game->statusState == STATUS_STATE_MENU) {
		...
		for (int i = 0; i < 75; i++)
		{
			...
			if (game->morcego1[i].vivo == 1) {
				SDL_Rect morcegoRect = { game->scrollx + game->morcego1[i].x*1.0f,game->morcego1[i].y,(float)40,(float)40 };
		...
		SDL_Rect hud13Rect = { game->scrollx+10100,(float)480,(float)250,(float)200 };
		...
		for (int i = 0; i < MAX_TIROS; i++) if(shot[i]){
			...
			SDL_Rect rect = {shot[i]->xt,shot[i]->yt,(float)10,(float)10 };
