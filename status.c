...
void draw_status_fim(GameState *game) {
	...
	if (game->sobvoltarfim == 1 || game->sobsairfim == 1) {
		Mix_PlayChannel(-1, game->menusom, 0);
	...
	if (game->sobvoltarfim >= 1) {
		SDL_Rect volt88Rect = { 119,559,101,101 };
		SDL_RenderCopy(renderer, game->voltarfim, NULL, &volt88Rect);
	}

	if (game->sobsairfim >= 1) {
		SDL_Rect sairgo66Rect = { 1039,559,101,101 };
		SDL_RenderCopy(renderer, game->sairgoi3, NULL, &sairgo66Rect);
	}
